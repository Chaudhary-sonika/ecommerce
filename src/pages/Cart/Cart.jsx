           ...
           cartItem?.map((item)=>{
            ...
            return(
                <div key={_id} className="CartProduct">
                    ...
                    <button  onClick={()=>RemoveFromCart(_id)} className="removeBtn"><FaRegTrashAlt/></button>
                    </div>
                    <h4>Price: Rs.{price}</h4>
                    <div className="cartQty">
                        <h4>Quantity: </h4>
                         <button onClick={()=>QuantityInCart(_id, "decrement")} className="qtyBtn">-</button><h3>{qty}</h3><button onClick={()=>QuantityInCart(_id, "increment")} className="qtyBtn">+</button>
                    </div>
